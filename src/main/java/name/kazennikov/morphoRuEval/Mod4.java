...
import name.kazennikov.common.MurmurHash3;
...
public class Mod4 {
    ...
    public Mod4(Lemmer lemmer) throws Exception {
        this.lemmer = lemmer;

        this.bits = 10;
        this.seeds = new int[200];
        this.dim = this.seeds.length * (1 << bits);

        Random r = new Random(0xbeefdead);
    ...
    public static class MCSolverSimple extends AbstractHKMulticlassCS {

        final int singleDim;
        public MCSolverSimple(MulticlassProblem problem, int singleDim, double c, double eps, int maxIter) {
            super(problem, problem.classes() * singleDim, c, eps, maxIter);
        ...
        @Override
        public int h(int v, int x) {
            return v * singleDim + x;
        }
    }

    public void train(List<Sentence> train) throws Exception {
        ...
        ThreadPoolExecutor executor = ThreadPoolUtils.newBlockingThreadPoolExecutor(6, 6, 300, TimeUnit.SECONDS, 4);
        ...
        models.add(executor.submit(new Callable<double[]>() {
            ...
            public double[] call() throws Exception {
                ...
                MCSolverSimple trainer = new MCSolverSimple(p, dim, c, 0.1, 1000);
                trainer.solve();
    ...
    public Instance hashFeats(List<String> feats) {
        ...
        int[] coords = new int[feats.size()];

        for(int i = 0; i < feats.size(); i++) {
            coords[i] = MurmurHash.hash32(feats.get(i)) >>> 1;
        }

        int[] mins = new int[seeds.length];

        for(int i = 0; i < seeds.length; i++) {
            int min = Integer.MAX_VALUE;

            for(int j = 0; j < coords.length; j++) {
                int v = MurmurHash.hash(coords[j], seeds[i]);
    ...
    public double dot(double[] w, int sdim, int cls, Instance x) {
        double d = 0.0;
        for(int i = 0; i < x.size(); i++) {
            d += w[sdim * cls + x.indexAt(i)] * x.valueAt(i);
        }
    ...
    public Lemmer.Parse predict(List<Lemmer.Parse> parses, Instance x) {
        ...
        for(String feat : set) {
            ...
            double score = dot(w, dim, featId, x);
...
//                }
    ...
    public static void main(String[] args) throws Exception {
        ...
        if(trainModel) {
            mod4.train(train);
            mod4.writeModel(new File("morphoRuEval.mod4.dat"));
        }




        mod4.readModel(new File("morphoRuEval.mod4.dat"));

        mod4.evalNG("morphoRuEval.mod4.test", test);
        mod4.evalNG("morphoRuEval.mod4.train", train);

        /*mod4.evalNG("morphoRuEval.mod4.syntagrus", CorpusReader.iter(new File("morphoRuEval/syntagrus_full.ud")));
        mod4.evalNG("morphoRuEval.mod4.rnc", CorpusReader.iter(new File("morphoRuEval/RNCgoldInUD_Morpho.conll")));
        mod4.evalNG("morphoRuEval.mod4.oc", CorpusReader.iter(new File("morphoRuEval/unamb_sent.txt")));*/
